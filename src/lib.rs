...
impl State {
    ...
    async fn new(window: Window) -> Self {
        ...
        let num_indices_2 = INDICES_2.len() as u32;

        let change_shape = false;
    ...
    fn input(&mut self, event: &WindowEvent) -> bool {
        match event {
            ...
            WindowEvent::KeyboardInput {
                ...
                self.change_shape = *state == ElementState::Released;
                true
            }
            _ => false,
        }
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
...

        {
            ...
            // NEW!
            render_pass.set_pipeline(match self.use_color {
                true => &self.challenge_render_pipeline,
                false => &self.render_pipeline,
            });
            render_pass.set_vertex_buffer(
                0,
                match self.change_shape {
                    false => self.vertex_buffer_2.slice(..),
                    true => self.vertex_buffer.slice(..),
                },
            );
            render_pass.set_index_buffer(
                match self.change_shape {
                    false => self.index_buffer_2.slice(..),
                    true => self.index_buffer.slice(..),
                },
                wgpu::IndexFormat::Uint16,
            );

            render_pass.draw_indexed(
                0..match self.change_shape {
                    false => self.num_indices_2,
                    true => self.num_indices,
                },
                0,
                0..1,
            );
...
const VERTICES_2: &[Vertex] = &[
    ...
    Vertex {
        position: [0.64147372, 0.5347359, 0.0],
        color: [0.5, 0.0, 0.2],
    }, // F
];
